# 🔥 Melted - Text Editor with Compare

**A sleek, minimalist text editor with powerful diff comparison built by Melty!**

![Version](https://img.shields.io/badge/version-1.0.0-blue)
![React](https://img.shields.io/badge/React-19-61DAFB)
![Monaco](https://img.shields.io/badge/Monaco-Editor-blue)

---

## ✨ Features

- 📝 **Tabbed Editing** - Work with multiple files at once
- 🔍 **Side-by-Side Diff** - Compare any two tabs instantly
- ✏️ **Editable Diffs** - Make changes directly in compare view
- 🎨 **6 Beautiful Themes** - MELTY, Obsidian, Dracula, Nord, Monokai, GitHub Dark
- 🔤 **Font Options** - JetBrains Mono, Fira Code, Source Code Pro
- 💾 **Auto-Save** - Your work persists in your browser
- 📂 **Drag & Drop** - Drop files to open them
- 🌐 **Live Language Detection** - Auto-detects syntax as you type

---

## 🚀 Get Started

### 🌐 Web Version
**[Launch Melted in your browser →](https://melty1000.github.io/-Melty-Melted-Text-Editor/)**

No installation required - just click and start editing!

### 💻 Desktop App

1. Go to [**Releases**](https://github.com/Melty1000/-Melty-Melted-Text-Editor/releases)
2. Download `Melted.1.0.0.exe` (portable) or `Melted.Setup.1.0.0.exe` (installer)
3. Run it!

---

## 🎯 How to Use

- Click **+** to create a new tab
- **Double-click** a tab name to rename it
- **Drag & drop** files onto the app to open them
- Click **Compare** → select a file to diff against current tab
- Edit either side directly in compare view!

---

## 🎨 Themes

| Theme | Description |
|-------|-------------|
| **MELTY** | Golden accent, dark background (default) |
| **Obsidian** | Classic dark editor look |
| **Dracula** | Purple and pink tones |
| **Nord** | Cool arctic blues |
| **Monokai** | Vibrant syntax colors |
| **GitHub Dark** | GitHub's dark theme |

---

## 📜 Usage Terms

**© 2026 Melty. All Rights Reserved.**

**You CAN:** ✅ Use it for your projects • Share the GitHub link

**Please DON'T:** ❌ Redistribute • Claim as your own • Sell it

Want to use it commercially? Just ask! 😊

---

## 💝 Support Me!

[![Discord](https://img.shields.io/badge/Discord-@Melty1000-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://discord.gg/8EfuxXgVyT)
[![Ko-fi](https://img.shields.io/badge/Ko--fi-Support_Me-F16061?style=for-the-badge&logo=ko-fi&logoColor=white)](https://ko-fi.com/melty1000)
[![Twitch](https://img.shields.io/badge/Twitch-melty1000-9146FF?style=for-the-badge&logo=twitch&logoColor=white)](https://www.twitch.tv/melty1000)
[![YouTube](https://img.shields.io/badge/YouTube-@melty__1000-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://www.youtube.com/@melty_1000)
[![GitHub](https://img.shields.io/badge/GitHub-Melty1000-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Melty1000)
[![X](https://img.shields.io/badge/X-@melty1000-000000?style=for-the-badge&logo=x&logoColor=white)](https://x.com/melty1000)

---

*Made with ❤️ and caffeine by Melty*
